...
router.put('/:id', verify, async (req, res) => {
    ...
    if(!foundUser) return res.status(404).json({message:"That user cannot be found"});

    const canEdit = (validUser === req.params.id);
    if(!canEdit) return res.status(401).json({message:"You can only edit your own account!"});
...
router.delete('/:id', verify, async (req, res) => {
    ...
    try{
        ...
        res.status(200).json({message: "User and posts have been deleted"});
    } catch(err) {
        res.status(500).jason(err)
    }
