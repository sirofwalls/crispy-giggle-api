...
router.put('/:id', verify, async (req, res) => {
    ...
    if (!post) return res.status(404).json("That post cannot be found");

    const canEdit = (validUser.username === post.author);
    if(!canEdit) return res.status(401).json("You can only edit your own post!");
...
router.delete('/:id', verify, async (req, res) => {
    ...
    const canEdit = (validUser.username === post.author);
    if(!canEdit) return res.status(401).json("You can only delete your own posts!");
